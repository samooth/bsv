...
class Script extends Struct {
  ...
  toString (opts={ noOp:false}) {
    ...
    for (let i = 0; i < this.chunks.length; i++) {
      ...
      const opCodeNum = chunk.opCodeNum
      if (!chunk.buf) {
        if (OpCode.str[opCodeNum.num] !== undefined || OpCode.noopstr[opCodeNum.num]!== undefined) {
          console.log("num opcode:",opCodeNum)
          str =  opts.noOp ? str + ' ' + new OpCode(opCodeNum.num).toNoOpString():  str + ' ' + new OpCode(opCodeNum.num).toString()
        } else {
          console.log("->",OpCode.str[opCodeNum.num],  OpCode.noopstr[opCodeNum])
          console.log("no es opcode en "+i, chunk, opCodeNum)
        ...
        if (
          opCodeNum === OpCode.OP_PUSHDATA1 ||
           ...
           str =  opts.noOp ? str + ' ' + new OpCode(opCodeNum).toNoOpString():  str + ' ' + new OpCode(opCodeNum).toString()
        }
        str = str + ' ' + chunk.len
  ...
  /**
     * Output the script to the script string format used in bitcoind data tests.
     */
  toAsmString (opts={noOp:false}) {
    var str = ''
    for (var i = 0; i < this.chunks.length; i++) {
      var chunk = this.chunks[i]
      str += this._chunkToString(chunk, opts)
    }

    return str.substr(1)
  }

  _chunkToString (chunk, opts={noOp: false}) {
