...
class Script extends Struct {
  ...
  toString (opts={ noOp:false}) {
    ...
    for (let i = 0; i < this.chunks.length; i++) {
      const chunk = this.chunks[i]
      const opCodeNum = chunk.opCodeNum.hasOwnProperty("num") ? chunk.opCodeNum.num : chunk.opCodeNum
      if (!chunk.buf) {
        if (OpCode.str[opCodeNum] !== undefined || OpCode.noopstr[opCodeNum]!== undefined) {
          console.log("num opcode:",opCodeNum)
          str =  opts.noOp ? str + ' ' + new OpCode(opCodeNum).toNoOpString():  str + ' ' + new OpCode(opCodeNum).toString()
        } else {
          console.log("->",OpCode.str[opCodeNum],  OpCode.noopstr[opCodeNum])
